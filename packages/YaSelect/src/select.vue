<template>
  ...
  >
    ...
    >
      <span v-if="collapseTags && selected.length && selectedWaitShow">
        <el-tag
          v-for="item in selected.slice(0, collapseTagsMax)"
        ...
        >
          <span class="el-select__tags-text"
            >+ {{ selected.length - collapseTagsMax }}</span
          >
        </el-tag>
      </span>
      <transition-group
        @after-leave="resetInputHeight"
        v-if="!collapseTags && selectedWaitShow"
      >
        <el-tag
...
export default {
  ...
  methods: {
    ...
    setSelected() {
      ...
      this.selectedWaitShow = false;
      this.$nextTick(() => {
        this.selectedWaitShow = true;
        this.resetInputHeight();
      });
    },
    ...
    changeCollapseTags() {
      this.$nextTick(() => {
        ...
        // 动态修改
        const tags = this.$refs.tags;
        let selectedValue = this.selected.map(item => item.currentLabel);
        // 按照文字
        let spanWidth = [];
        let spanTotallWidth = 0;
        for (let index in selectedValue) {
          spanWidth.push(selectedValue[index].length * 12 + 34);
          // spanTotallWidth += selectedValue[index].length * 12 ;
        }
        spanTotallWidth = spanWidth.reduce((a, b) => a + b);

        let collapseTagsIndex = 0;
        spanWidth.reduce((total, currentValue, currentIndex) => {
          if (tags.offsetWidth > total + 54) {
            collapseTagsIndex = currentIndex;
            return total + currentValue;
          }
        }, 0);
        this.collapseTagsMax = collapseTagsIndex == 0 ? 1 : collapseTagsIndex;
        this.collapseTags =
          tags.offsetWidth > spanTotallWidth + 30 || selectedValue.length == 1
            ? false
            : true;
        this.resetInputHeight();
      });
    ...
    handleOptionSelect(option, byClick) {
      if (this.multiple) {
        ...
        if (optionIndex > -1) {
          ...
          if (option.label === "全部") {
            this.checkAllValue = option.value;
            value = [option.value];
          } else {
            value = value.filter(item => item && item != this.checkAllValue);
    ...
    watchSize() {
      ...
      erd.listenTo(this.$refs.tags, element => {
